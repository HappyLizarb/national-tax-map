...
assert.ok(html.indexOf('id="netPositionSummary"') > html.indexOf('aria-labelledby="gapTitle"') && html.indexOf('id="netPositionSummary"') < html.indexOf('id="incomeTaxTiersTitle"'));
...
assert.ok(html.indexOf('class="tax-rate-detail detail-disclosure"') > html.indexOf('id="taxEstimateSection"') && html.indexOf('class="tax-rate-detail detail-disclosure"') > fiscalDetailStart);
assert.ok(html.indexOf('id="everydayCostsRows"') > html.indexOf('id="taxEstimateSection"'));
...
for (const pattern of [/<details class="household-tax-detail detail-disclosure"/, /<details class="tax-rate-detail detail-disclosure"/, /<summary>/]) assert.match(html, pattern);
assert.doesNotMatch(html, /<details[^>]+detail-disclosure[^>]+open/);
...
assert.ok(html.indexOf('id="dataBasis"') > html.indexOf('class="methodology-disclaimer"'));
assert.doesNotMatch(html.slice(fiscalDetailStart, html.indexOf("</main>")), /panel-footer|id="dataBasis"/);
for (const pattern of [/DepartmentData\.loadSummary/, /DepartmentData\.loadDetail/, /reconcileStateArchive/, /updateNetPosition\(canonical\)/, /const canonical = model\.scopeData\(state\.scope, state\.layer === "federal" \? "itemized" : "financial"\)/, /const allocation = model\.scopeData\(state\.scope, state\.layer === "federal" \? "itemized" : "archive"\)/, /scope === state\.scope && scope !== "United States"/, /themePreference\.addEventListener\("change", followSystemTheme\)/, /color\("--map-neutral"\)/, /easeCubicInOut/, /Working individual/]) assert.match(app, pattern);
assert.doesNotMatch(app, /animatePanelChange|element\.animate/);
assert.match(app, /const costsOpen = \$\("\.consumer-costs"\)\?\.open[\s\S]*if \(costsOpen\) \$\("\.consumer-costs"\)\.open = true/);
assert.match(app, /everydayCostsRows[\s\S]*renderEverydayCosts/);
...
assert.match(app, /nestedSourceBreakdowns/);
...
assert.match(app, /flatSingleton \? "leaf account · redundant one-row wrapper removed"/);
assert.match(app, /rowHasExactBreakdown/);
assert.match(app, /match\.rows\.length === 1[\s\S]*leaf account[\s\S]*<details class="item-breakdown"/);
assert.match(app, /rowHtml \+ nestedSourceBreakdowns\(detail, row, rendered\)/);
assert.match(app, /matches\.length === 1.*receiptAccountHtml/s);
assert.match(app, /mapMetric\(name, "stateGovernment", "balance", "financial"\)/);
assert.match(app, /targetTransform = "translate\(0 0\) scale\(1\)"/);
assert.match(app, /fitExtent\(\[\[18, 14\], \[882, 462\]\]/);
...
assert.match(app, /financial\.changeInNetPosition\) \+ " change in net position"/);
assert.match(app, /financial \? "Resources & expenses" : "Revenue–spending gap"/);
assert.match(app, /canonical\.kind === "federal" \? "Federal outlays" : status/);
assert.match(app, /setText\("#revenueBarLabel", "GAAP Resources"\)[\s\S]*setText\("#spendingBarLabel", "GAAP Spending"\)/);
assert.doesNotMatch(app, /annual change in net position|GAAP resources and expenses/);
