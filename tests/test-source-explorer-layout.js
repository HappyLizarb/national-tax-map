...
assert.match(bundle, /styles\/polish\.css\?v=6[\s\S]*@keyframes reveal-up[\s\S]*body:has\(\.entry-disclaimer\[open\]\)[\s\S]*animation-play-state: paused/);
...
assert.match(bundle, /prefers-reduced-motion: reduce[\s\S]*\.fiscal-detail \.section-heading h3\s*\{\s*font-size: clamp\(28px, 3vw, 40px\)/);
